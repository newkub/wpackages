...
import { getUsedIconNames, generateIconCss, resolveIcons } from './generator/icons'
...
export async function generateCss(classes: Set<string>, userOptions: UserOptions = {}): Promise<string> {
  ...
  const tailwindCss = await buildTailwindCssWithCandidates(finalCssInput, options, [...finalClasses].filter(c => !c.startsWith('icon-[')), config)
