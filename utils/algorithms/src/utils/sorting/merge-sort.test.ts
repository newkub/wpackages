...
describe('mergeSort', () => {
  it('should sort an array of numbers', () => {
    ...
    expect(mergeSort(arr)).toEqual([11, 12, 22, 25, 34, 64, 90]);
  });

  it('should sort an array of strings', () => {
    const arr = ['banana', 'apple', 'cherry', 'date'];
    expect(mergeSort(arr)).toEqual(['apple', 'banana', 'cherry', 'date']);
  });
  ...
  it('should handle an array with duplicate elements', () => {
    ...
    expect(mergeSort(arr)).toEqual([1, 2, 2, 5, 5, 8]);
  });
  ...
  it('should sort an array with negative numbers', () => {
    ...
    expect(mergeSort(arr)).toEqual([-30, -10, 0, 5, 20]);
  });
});
