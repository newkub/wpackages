...
const activeContainer = computed(() =>
	containers.value.find((c: { id: string }) => c.id === activeContainerId.value),
);
...
const handleQuickAction = async (action: string) => {
	...
	switch (action) {
		...
		case "files": {
			...
			files.forEach((f: { name: string }) => output.value.push(`  ${f.name}`));
			break;
		}
		case "ports": {
	...
	<div class="app">
		<header class="header">
			...
			<div class="header-actions">
				<NuxtLink to="/bench" class="btn-action" aria-label="Open bench viewer">📊 Bench Viewer</NuxtLink>
				<button type="button" @click="showCreateModal = true" class="btn-primary" aria-label="Create new container">
					+ New Container
				</button>
			</div>
		</header>
			...
			<section class="content">
				...
				<div v-else class="container-view">
					<div class="container-header">
						...
						<span class="status" :class="activeContainer.status">{{ activeContainer.status }}</span>
					</div>
					...
					<div class="quick-actions">
						...
						<button type="button" @click="handleQuickAction('dev')" class="btn-action">🔥 Dev</button>
						<button type="button" @click="handleQuickAction('build')" class="btn-action">🔨 Build</button>
						<button type="button" @click="handleQuickAction('test')" class="btn-action">🧪 Test</button>
						<button type="button" @click="handleQuickAction('files')" class="btn-action">📂 Files</button>
						<button type="button" @click="handleQuickAction('ports')" class="btn-action">📍 Ports</button>
					</div>
					...
					<div class="command-input">
						...
						/>
						<button type="button" @click="handleExecute" class="btn-primary" aria-label="Run command">Run</button>
					</div>
				</div>
			</section>
		</main>
